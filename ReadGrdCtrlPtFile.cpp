...
bool ReadGrdCtrlPtFile(const char* strFile, GCP** pGCP, int* nGCP) {
	...
	if (*nGCP) {
		...
		for (int i = 0; i < *nGCP; i++) {
			fscanf_s(fp, "%d %lf %lf %lf\n", &(*pGCP)[i].ID,&(*pGCP)[i].x, &(*pGCP)[i].y, &(*pGCP)[i].z);
		}
	}
...
bool WriteGCPAsTextFile(const char* strFile, GCP* pGCP, int nGCP) {
	FILE* fp = nullptr;
	errno_t err = fopen_s(&fp, strFile, "wt");
	if (!fp) {
		printf("Error: can not open file %s!\n", strFile);
		return false;
	}
...
void main() {
	char strTxtFile[] = "E:\\gcp.txt";
	char strBinFile[] = "E:\\gcp.bin";
